      ...
      <div className="max-w-3xl mx-auto">
        ...
        {!company && (
          <div className="mb-6 flex items-start gap-3 bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3.5">
            ...
            <div>
              <p className="text-sm font-semibold text-amber-400">No approved company found</p>
              <p className="text-xs text-amber-400/70 mt-0.5">
                You need a registered and approved company to post jobs.{" "}
                <Link href="/dashboard/recruiter/company/register" className="underline">
        ...
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <div
                      ...
                      className={`relative w-9 h-5 rounded-full transition-colors duration-200 ${
                        isRemote ? "bg-[#6b5ce7]" : "bg-white/10"
                      }`}
                    >
                      <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${
                        isRemote ? "left-[18px]" : "left-0.5"
                      }`} />
                    </div>
                  </label>
                </div>
            ...
            <div className="flex items-center gap-4 p-4 bg-[#0d0d15] border border-white/8 rounded-xl">
              {company ? (
                <>
                  ...
                  <span className="ml-auto text-[11px] font-semibold text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 rounded-full px-2.5 py-0.5">
                    Approved
                  </span>
                </>
              ) : (
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-2 pb-8">
            ...
            <button
              type="submit"
              disabled={loading || success || !company}
              className="w-full sm:w-auto flex items-center justify-center gap-2
                         bg-[#6b5ce7] hover:bg-[#7c6af7]
